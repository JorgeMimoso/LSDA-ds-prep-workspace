...
class MinesweeperGame:
    ...
    def check_if_mine(self, x: int, y: int) -> bool:
        ...
        True
        """
        return bool(self.mines[x][y]) #insert your code here
    ...
    def count_mines_around(self, x: int, y: int) -> int:
        ...
        1
        """
        total_mines = sum([self.mines[pos_x][pos_y] for pos_x,pos_y in self.get_surrounding_cells(x,y)])

        ''' -> kind of ignored available methods here
        for m in range(len(self.mines)):
            for n in range(len(self.mines[0])):
                if abs(m-x)<=1 and abs(n-y)<=1 and ((x,y)!=(m,n)):
                    total_mines+=self.mines[m][n]
        '''

        return total_mines #insert your code here
    ...
    def click_around(self, x: int, y: int):
        ...
        be helpful. 
        """
        #if self.count_mines_around == 0 -> this check is already done in MAIN
